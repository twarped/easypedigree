(() => {
    var dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(
        ...
        , "ROOTNAMES": (() => {
            window.ROOTNAMES = []
        })(), "DESCROOT": (() => { window.DESCROOT = [document.querySelector("#main-content-section > tree-app")
            ...
            .shadowRoot.querySelector("#pages > tree-pedigree")
            .shadowRoot.querySelector("#pedigree")
            .shadowRoot.querySelectorAll("#pedigree > div:nth-child(1) > pedigree-couple-renderer.desc-node"))
            .filter(c => { try { 
                return c.shadowRoot.querySelector("div.couple-wrapper")
                .parentNode.querySelector("button.append-pedigree").title.includes("Collapse") 
            } catch (e) { return false } })]; 
        return window.DESCROOT; })().map(couple => { 
            return couple.coupleId.slice(0, couple.coupleId.includes(".") ? -4 : -2) }).reduce((p, c, i) => ({
                ...p, [window.DESCROOT[i].attributes["data-test-couple-ahnentafel"].nodeValue]: c 
            }), {}) })); 
